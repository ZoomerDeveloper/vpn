...
# Определяем сеть (можно указать через переменную окружения WG_NETWORK)
# Для второго сервера рекомендуется использовать другую подсеть (например, 10.10.0.0/24)
if [ -n "$WG_NETWORK" ]; then
    NETWORK="$WG_NETWORK"
    PRIVATE_IP=$(echo "$NETWORK" | cut -d'/' -f1 | sed 's/\.0$/\.1/')
else
    # По умолчанию используем 10.0.0.0/24, но можно изменить
    NETWORK="10.0.0.0/24"
    PRIVATE_IP="10.0.0.1"
fi
