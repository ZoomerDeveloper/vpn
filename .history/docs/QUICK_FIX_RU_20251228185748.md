...
## Рекомендуемая конфигурация для РФ

Минимальный конфиг клиента, оптимизированный для работы в РФ:
...
**Важные параметры:**
- `DNS = 1.1.1.1` - Cloudflare DNS, лучше всего работает в РФ
- `MTU = 1280` - минимальный безопасный MTU для избежания фрагментации
- `Endpoint = ...:443` - порт 443/UDP обычно не блокируется провайдерами
