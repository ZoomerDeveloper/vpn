...
if [ -z "$INPUT_ID" ]; then
    echo -e "${YELLOW}Использование: bash diagnose-vpn-ru-detailed.sh [API_URL] USER_ID_OR_TELEGRAM_ID${NC}"
    echo "Пример: bash diagnose-vpn-ru-detailed.sh http://localhost:3000 USER_ID"
    echo "Или: bash diagnose-vpn-ru-detailed.sh http://localhost:3000 TELEGRAM_ID"
    exit 1
fi

# Определяем это USER_ID (UUID) или Telegram ID (число)
if echo "$INPUT_ID" | grep -qE "^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$"; then
    USER_ID="$INPUT_ID"
    echo -e "${CYAN}Используется USER_ID: ${USER_ID:0:8}...${NC}"
else
    # Это Telegram ID, получаем USER_ID
    echo -e "${CYAN}Ищем пользователя по Telegram ID: $INPUT_ID${NC}"
    USER_RESPONSE=$(curl -s "${API_URL}/users/telegram/${INPUT_ID}" 2>/dev/null)
    
    if [ -z "$USER_RESPONSE" ] || echo "$USER_RESPONSE" | grep -q "404\|Cannot GET"; then
...
" 2>/dev/null)
    ...
    if [ -z "$USER_ID" ]; then
        echo -e "${RED}❌ Не удалось получить USER_ID${NC}"
        exit 1
    fi
    
    echo -e "${GREEN}✓ Найден USER_ID: ${USER_ID:0:8}...${NC}"
fi
...
if [ -z "$PEERS" ] || echo "$PEERS" | grep -q "\[\]\|404\|Cannot GET"; then
    echo -e "${RED}❌ У пользователя нет peer'ов или ошибка получения${NC}"
    exit 1
fi

# Находим активный peer
